...
For this simple example, we will read the contents of the [cereal.csv](cereal.csv)
file, which is one of the fun beginner datasets provided on [Kaggle](https://www.kaggle.com/crawford/80-cereals/data).
The script [generate_tabular.py](generate_tabular.py) provided here can
be run locally to show the table. First, let's look at the input arguments:

```bash
./generate_tabular.py --help
usage: generate_tabular.py [-h] [--version] [--no-header] [--delim DELIM]
                           [input_file]

Pyout Simple Example

positional arguments:
  input_file     csv file to read into table

optional arguments:
  -h, --help     show this help message and exit
  --version      show version of pyout
  --no-header    data does not have a header
  --delim DELIM  delimiter to split tabular file (defaults to comma)
```
...
Fruit & Fibre Dates; Wal... P   C    120      3       2   160    5     12    10     200    25       3     1.25   0.67 40.917047
...
Muesli Raisins; Dates; &... R   C    150      4       3   95     3     16    11     170    25       3     1      1    37.136863
Muesli Raisins; Peaches;... R   C    150      4       3   150    3     16    11     170    25       3     1      1    34.139765
...
And the colors can be seen here:

![table.png](table.png)

Note that you can do customization for coloring, width, and other styling. The table
here simply makes text items "110" red, and "100" green for simple examples. A more
detailed example will be developed to include details on custom styling and
usage.
